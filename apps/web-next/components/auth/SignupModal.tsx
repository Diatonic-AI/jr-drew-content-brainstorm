...
export function SignupModal({ open, onOpenChange, answers, onSuccess, prefillEmail, fromQuiz }: SignupModalProps) {
  ...
  const ensurePlaces = useCallback(async () => {
    ...
    if (!apiKey) {
      console.warn('Missing NEXT_PUBLIC_GOOGLE_PLACES_API_KEY for address autocomplete')
      return
    }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{fromQuiz ? 'Submit your tailored productivity profile' : 'Create your Diatonic AI account'}</DialogTitle>
          <DialogDescription>
            We&apos;ll use your answers to personalise your workspace, automations, and onboarding experience.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(submit)} className="space-y-4">
          ...
          <Field label="Anything we should prep before our kickoff?" optional error={errors.message?.message}>
            <Textarea placeholder="Share context, goals, or integrations to prioritise…" {...register('message')} />
          </Field>
          {answers && (
            <div className="rounded-2xl border bg-muted/30 p-4 text-sm">
              <h4 className="mb-2 font-medium">Quiz snapshot</h4>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 text-sm">
              ...
              />
              <span>I agree to receive Diatonic resources, launch briefings, and productivity strategies.</span>
            </label>
            <label className="flex items-center gap-3 text-sm">
              <Checkbox
          ...
          <DialogFooter>
            <Button type="submit" className="w-full" disabled={isSubmitting}>
              {fromQuiz ? (isSubmitting ? 'Submitting…' : 'Submit my answers') : isSubmitting ? 'Creating account…' : 'Create account'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
