...
export function QuizFunnel({ autoStart }: QuizFunnelProps) {
  ...
  const handleAdvance = (autoDueToTimeout = false) => {
    ...
    () => [
      {
        ...
        description: 'Your responses orchestrate a dashboard, focus rituals, and automation templates tuned to how you operate.'
      },
      {
        title: 'Team onboarding blueprint',
        description: 'We translate quiz insights into a rollout playbook so collaborators adopt Diatonic in under a week.'
      },
      {
        title: 'Momentum in 60 seconds',
        description: 'After signup you’ll receive a ready-to-launch experience designed to return a full hour to your day.'
      }
    ],
    []
  )
  ...
  return (
    <>
      <section className="relative overflow-hidden rounded-3xl border bg-gradient-to-br from-background via-background to-muted p-8 sm:p-12 shadow-xl">
        ...
        <div className="relative z-10 mx-auto max-w-4xl space-y-8 text-center">
          <header className="space-y-4">
            ...
            <h1 className="text-4xl font-semibold sm:text-5xl">Discover your Diatonic focus profile</h1>
            <p className="mx-auto max-w-2xl text-base text-muted-foreground">
              Answer seven fast prompts so we can auto-build the rituals, dashboards, and AI assist flows that earn you back time immediately.
            </p>
          </header>
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="grid gap-4 sm:grid-cols-3">
                  {summaryCards.map(card => (
                    <div key={card.title} className="rounded-2xl border bg-background/60 p-5 text-left shadow-sm backdrop-blur">
            ...
            {stage === 'quiz' && currentQuestion && (
              ...
              >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <div className="flex items-center gap-1 rounded-full bg-muted/60 px-3 py-1 text-xs font-medium text-foreground">
                        {remaining > 0 ? <PlayCircle className="h-4 w-4" /> : <PauseCircle className="h-4 w-4 text-destructive" />}
                        <span>{remaining}s</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex-1 basis-full overflow-hidden rounded-full bg-muted sm:basis-auto sm:max-w-xs">
                    <div
                ...
                <div className="rounded-3xl border bg-background/80 p-6 shadow-lg backdrop-blur">
            ...
            {stage === 'summary' && (
              ...
              >
                <div className="rounded-3xl border bg-background/90 p-6 shadow-lg backdrop-blur">
                  <h2 className="text-3xl font-semibold">Your productivity blueprint is ready</h2>
                  <p className="mt-3 text-muted-foreground">
                    We translated your responses into a tailored onboarding plan. Save it by creating your account, or talk with our team to review the roadmap live.
                  </p>
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {summaryCards.map(card => (
                    <div key={card.title} className="rounded-2xl border bg-muted/30 p-5 shadow-sm">
